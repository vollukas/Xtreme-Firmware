...
static void updater_main_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(model->failed) {
        ...
        canvas_draw_str_aligned(canvas, 85, 16, AlignCenter, AlignTop, "Xtreme FW");
        canvas_draw_icon(canvas, -4, -22, &I_ActiveConnection_50x64);
        canvas_draw_icon(canvas, 3, 6, &I_FaceCharging_29x14);
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 85, 6, AlignCenter, AlignTop, "Installing");
        canvas_draw_str_aligned(
            canvas, 64, 51, AlignCenter, AlignTop, furi_string_get_cstr(model->status));
        //canvas_draw_icon(canvas, 4, 5, &I_Updating_32x40);
        elements_progress_bar(canvas, 45, 29, 80, (float)model->progress / 100);
    }
}
